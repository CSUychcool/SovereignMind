...
- 文档上传（txt/md，**可拖拽/选文件/粘贴三合一**，带上传阶段进度）→ 切块 → **本地 embedding 向量化**（Ollama `/api/embed`，与聊天后端解耦）→ 入库；
...
### 3. 语音全链路 · 按网络分档、断网也有降级
一条「说→听」链路，联网/离线都给你选：

- **在线（默认，最自然）**：**edge-tts**（微软，晓晓/晓伊/云希），内容零误差、语气自然；
- **离线本地（可选音色）**：**CosyVoice2**（阿里开源，本地 GPU）音色库——内置「林志玲 / 我的声音」，把任意 `.wav+.txt` 丢进 `voice/cosyvoice-voices/` 就是新音色；
- **离线且部署失败**：**piper**（纯离线轻量）→ **浏览器系统语音**，逐级兜底不哑声；
- **ASR**：浏览器实时识别优先 → 失败自动回退**本机 whisper.cpp**；
- **读得准的机制**：CosyVoice 采用 **cross_lingual**（LLM 只吃音频特征、不读 prompt 文本），内容逐字对齐目标文本；采样 `0.3/10` 平衡自然度与稳定性；
- **SSE 整段音频** · WebAudio 播放 · **暂停/继续/调速** · 回答自动朗读。

### 4. 音色库：一个 wav 就是一个音色
- 每个音色 = `cosyvoice-voices/<name>.wav + .txt`（零样本/克隆），`GET /voices` 暴露、前端下拉即换；
- 音色内容经 **whisper 回读对账**自动化验证，保证"念的就是屏幕上的字"。

### 5. 上下文全透明 · 看到模型"看见"了什么（🧠）
- 每次 `/api/chat` 都在 SSE 里带回**本次实际喂给模型的上下文清单**：系统人设 / RAG 资料 / 摘要 / 历史（装了 n 条·是否截断）/ 召回 / 提问，逐段 token 数与**全文**；
- 前端每条 AI 消息 **🧠**（或点用量条）→ 抽屉逐段展开查看；
- 只展示"当前这一次"、不落库不回放——云端不敢给的完整上下文，私有化全给你（对应 `docs/requirements.md` 需求二十四）。
...
index.html (单文件前端 · 深空主题)
   └─ http://localhost:9000 → llm-server (C++17 自研事件驱动 HTTP)
              ...
              │  ├─ /api/tts       → 本地 TTS: SSE 音频 (edge-tts在线→CosyVoice2→piper→系统)
...
- **语音**：录音转文字（浏览器→whisper 降级）；朗读 **edge-tts(在线)→CosyVoice2(本地音色)→piper→系统** 逐级降级；暂停/调速/自动读
...
- **上下文全透明**：UI 实时展示**当下拼接的上下文**——每次生成前"喂了什么给模型"所见即所得，逐段来源可追溯；云端不敢给的完整上下文，私有化部署全给你（`docs/requirements.md` 需求二十四）
...
语音：edge-tts(在线) · CosyVoice2(GPU 本地/音色库) · piper · whisper.cpp · 向量：进程内向量索引（预留 faiss 后端）
