...
| **采样定 `top_p=0.3/top_k=10`** | LLM 每步 `multinomial` 随机采样——过活跃内容走样、过保守语气呆。实测扫描（me 音色, whisper 回读保真度）：0.80/25→0.71、0.50/15→0.71、**0.30/10→0.84(最佳)**、0.15/5→0.69 → 取 0.30/10 平衡"自然语气"与"内容正确" |
